...
PYEOF
    ...
    # Use src/generated/orchestrator.py and src/handlers/ (generated by rsf generate)
    # These are the RSF-managed files; the root orchestrator.py is the legacy version.
    # The Lambda handler is "orchestrator.lambda_handler" so we package orchestrator.py
    # from src/generated/ as orchestrator.py at the zip root.
    cd "${SCRIPT_DIR}"
    zip -j "${BUILD_DIR}/function.zip" src/generated/orchestrator.py
    zip -r "${BUILD_DIR}/function.zip" rsf/ \
      -x "**/__pycache__/*" "**/*.pyc"
    # Add handlers from src/handlers/ (flat structure under handlers/)
    cd "${SCRIPT_DIR}/src"
    zip -r "${BUILD_DIR}/function.zip" handlers/ \
      -x "**/__pycache__/*" "**/*.pyc"
    cd "${SCRIPT_DIR}"
