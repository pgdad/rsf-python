    ...
    while current_state is not None:
        ...
        if current_state == 'CallPrimaryService':
            try:
                ...
                input_data = context.step(lambda _step_ctx: handler(input_data), 'CallPrimaryService')
            ...
            input_data = context.step(lambda _step_ctx: handler(input_data), 'VerifyResult')
            current_state = 'ServiceComplete'

        elif current_state == 'TryFallbackService':
            try:
                handler = get_handler('TryFallbackService')
                input_data = context.step(lambda _step_ctx: handler(input_data), 'TryFallbackService')
                current_state = 'VerifyResult'
            except Exception as _err:
            ...
            try:
                ...
                input_data = context.step(lambda _step_ctx: handler(input_data), 'HandleBadData')
                current_state = 'CallPrimaryService'
            except Exception as _err:
                if type(_err).__name__ in ['States.ALL'] or "States.ALL" in ['States.ALL']:
                    input_data = _apply_error_result(input_data, _err, '$.error')
            ...
            try:
                ...
                input_data = context.step(lambda _step_ctx: handler(input_data), 'RetryAfterThrottle')
                current_state = 'VerifyResult'
            except Exception as _err:
                if type(_err).__name__ in ['States.ALL'] or "States.ALL" in ['States.ALL']:
                    input_data = _apply_error_result(input_data, _err, '$.error')
