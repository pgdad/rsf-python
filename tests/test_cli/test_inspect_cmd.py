...
def test_inspect_with_arn_calls_launch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_inspect_no_arn_no_terraform_exits_1(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_inspect_discovers_arn_from_terraform(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    tf_dir.mkdir()
    (tf_dir / "terraform.tfstate").write_text(json.dumps({"version": 4, "outputs": {}}), encoding="utf-8")

    with patch("subprocess.run") as mock_run, patch("rsf.inspect.server.launch") as mock_launch:
...
def test_inspect_terraform_fails_exits_1(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_inspect_port_and_no_browser_flags(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_inspect_default_port_and_open_browser(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
