...
class TestEntrypointWorkflowFileForwarding:
    ...
    def test_deploy_receives_workflow_file(self):
        ...
        assert (
            "WORKFLOW_FILE" in first_line_after or "WORKFLOW_FILE" in content.split("DEPLOY_CMD=", 1)[1].split("\n")[0]
    ...
    def test_deploy_command_has_workflow_file_before_flags(self):
        ...
        for line in content.splitlines():
            if "DEPLOY_CMD=" in line and "rsf deploy" in line:
                ...
                assert "WORKFLOW_FILE" in line, f"DEPLOY_CMD line does not include WORKFLOW_FILE: {line.strip()}"
                break
        else:
