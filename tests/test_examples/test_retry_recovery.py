...
- CloudWatch log assertions for handler execution [VERF-02]

State types exercised: Task, Pass, Succeed, Fail (defined)

Happy path (no simulate_error): CallPrimaryService → VerifyResult
→ ServiceComplete (Succeed).
"""
...
class TestRetryRecoveryIntegration:
    ...
    EXAMPLE_DIR = EXAMPLES_ROOT / "retry-and-recovery"

    # No simulate_error → happy path through primary service
    EVENT = {
        "requestId": "test-001",
    ...
    def test_handler_log_entries(self, deployment, logs_client):
        """CloudWatch logs confirm primary service and verification (VERF-02).

        Expected path: CallPrimaryService → VerifyResult → ServiceComplete.
        """
